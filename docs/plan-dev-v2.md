...
- **Prochaine action** : commit + tag `v0.8.0-dynamic-pool` + release
  GitHub. Puis :
  * Échelon E — pool chaud (`min_available` pré-provisionnement)
  * Échelons F-G — GC orphelines au boot, quotas par utilisateur
  * Réduction RAM template (20 Go → 4 Go)
  * Optimisation boot VM (cloud-init minimal, ~15s au lieu de ~40s)
  * Systemd units pour host-agent et proxy
  * C2-E — brique innovation thèse IDPE : firewall sémantique XML,
    capacités CID-bound, attestation mutuelle IMA/EVM
