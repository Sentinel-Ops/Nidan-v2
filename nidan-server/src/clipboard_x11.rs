//! Propriétaire de la sélection X CLIPBOARD côté serveur (VM).
//!
//! Quand le serveur reçoit un presse-papier (filtré) du client, il doit le
//! rendre disponible aux applications de la VM : un Ctrl+V doit coller ce
//! contenu. En X11, cela suppose de **posséder** la sélection CLIPBOARD et de
//! répondre aux `SelectionRequest` émis par l'application qui colle.
//!
//! Ce module crée une fenêtre cachée dédiée, prend possession de CLIPBOARD,
//! et lance une boucle d'événements (thread dédié) qui sert le contenu courant
//! aux demandeurs (cibles UTF8_STRING / STRING / TEXT, et TARGETS).
...
impl ClipboardOwner {
    ...
    pub fn start(display: u32) -> Result<Self> {
        ...
        let (conn, screen_num) = x11rb::connect(Some(&display_str))
            .with_context(|| format!("connexion X11 (clipboard) sur {display_str}"))?;
